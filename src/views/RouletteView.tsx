...
export function RouletteView({ routes }: RouletteViewProps) {
  ...
  return <div className="flex-1 flex flex-col items-center justify-center px-6 pt-16 pb-28 w-full h-full">
    ...
    <div className="flex-1 w-full flex flex-col items-center justify-center"><div className="mb-8 text-center"><h1 className="text-3xl font-bold tracking-tight text-brand-text mb-2">Wohin geht die Fahrt?</h1><p className="text-brand-dim">{filteredRoutes.length} von {routes.length} Routen zur Auswahl</p></div>
      ...
      <motion.button onClick={spin} disabled={filteredRoutes.length === 0 || isSpinning} className="w-full mb-8 bg-brand-accent disabled:bg-brand-surface disabled:text-brand-dim text-black font-extrabold p-5 rounded-2xl uppercase tracking-wider text-lg border-none">{isSpinning ? 'Mischt...' : 'Tour wählen'}</motion.button>
    </div></div>;
}
