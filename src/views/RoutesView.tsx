...
export function RoutesView({ routes, addRoute, deleteRoute, updateRoute, importRoutes }: RoutesViewProps) {
  ...
  return <div className="flex-1 w-full max-w-2xl mx-auto flex flex-col h-full overflow-hidden">
    ...
    <div className="flex-1 overflow-y-auto px-6 pb-24 space-y-4" style={{ touchAction: 'pan-y' }}>
      ...
      <div className="grid grid-cols-2 gap-4"><select value={type} onChange={(e)=>setType(e.target.value as RouteType)} className="w-full bg-brand-light border border-brand-border rounded-[8px] p-[10px]">{ROUTE_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select>
      <div className="flex gap-2">{DIFFICULTIES.map(d=><button key={d} type="button" onClick={()=>setDifficulty(d)} className={`flex-1 py-1.5 px-3 rounded-[20px] text-[10px] uppercase font-bold ${difficulty===d?'bg-brand-accent/20 text-brand-accent border border-brand-accent/50':'bg-brand-accent/5 text-brand-accent opacity-50'}`}>{d}</button>)}</div></div>
